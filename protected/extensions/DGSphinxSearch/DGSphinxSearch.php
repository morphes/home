...
class DGSphinxSearch extends CApplicationComponent
{
    ...
    public function search($criteria = null)
    {
        ...
        return $this->initIterator($res, $this->criteria);
    }
    ...
    public function filters($filters)
    {
        ...
        if ($filters && is_array($filters)) {
            foreach ($filters as $fil => $vol) {
                // geo filter
                if ($fil == 'geo') {
                    $min = (float) (isset($vol['min']) ? $vol['min'] : 10);
    ...
    public function orderby($orders = null)
    {
        ...
        /*
         * TODO Remove this code after refactoring CSort
         */
        // set order by string or CSort object
        if ($orders) {
            if (is_string($orders)) {
                $this->client->SetSortMode(SPH_SORT_EXTENDED, $orders);
            } else if ($orders instanceOf CSort) {
