<?php
/**
 * Подключение к SQL бд из консольного приложения
 * включает переподключение в случае потери соединения
 */
class EDbConnection extends CDbConnection
{
	public  $reconnectTimeOut = 30;
	private $_reconnectCount = 0;
	public $maxReconnectCount = 500;
	...
	protected function open()
	{
		while (true) {
			try
			{
				...
				if ( (isset($e->errorInfo[1]) && $e->errorInfo[1] == 2006) || $e->getCode() == 2006 ) { // HAS GONE AWAY
					echo "\nReconnect {$this->connectionString}\n";
					continue;
				} elseif ( (isset($e->errorInfo[1]) && $e->errorInfo[1] == 2013) || $e->getCode() == 2013 ) { // LOST CONNECTION
					echo "\n LOST CONNECTION {$this->connectionString}\n";
					sleep($this->reconnectTimeOut);
					continue;
				} elseif ( $e->getCode() == 2003 ) { // HAS NO CONNECT
					echo "\n NO CONNECT {$this->connectionString}\n";
					sleep($this->reconnectTimeOut);
					continue;
				} elseif ($e->getCode() == 2002) { // can`t connect
					echo "\n CANNOT CONNECT {$this->connectionString}\n";
					sleep($this->reconnectTimeOut);
					continue;
				} else {
					throw $e;
...
class EDbCommand extends CComponent
{
	/**
	 ...
	 * for more details.
	 */
	public function __construct(EDbConnection &$connection,$query=null)
	{
		...
		while (true) {
			...
			catch(Exception $e)
			{
				...
				if ( $e instanceof PDOException ) {
					if ( (isset($e->errorInfo[1]) && $e->errorInfo[1] == 2006) || $e->getCode() == 2006 ) { // HAS GONE AWAY
						//sleep($this->_connection->reconnectTimeOut);
						echo "\nRECONNECT {$this->_connection->connectionString}\n";
						$this->_connection->setActive(false);
						$this->_connection->setActive(true);
						$this->cancel();
						continue;
					} elseif ( (isset($e->errorInfo[1]) && $e->errorInfo[1] == 2013) || $e->getCode() == 2013 ) { // LOST CONNECTION
						echo "\n LOST CONNECTION {$this->_connection->connectionString}\n";
						...
						echo "\n NO CONNECT {$this->_connection->connectionString}\n";
			...
			catch(Exception $e)
			{
				if ( $e instanceof PDOException ) {
					if ( (isset($e->errorInfo[1]) && $e->errorInfo[1] == 2006) || $e->getCode() == 2006 ) { // HAS GONE AWAY
						...
						sleep($this->getConnection()->reconnectTimeOut);
						$this->_connection->setActive(false);
						$this->_connection->setActive(true);
						$this->cancel();
						continue;
