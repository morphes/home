...
class VacanciesController extends AdminController
{
	...
	public function actionIndex()
	{
		...
		$count = Vacancies::model()->count($criteria);
	...
	/**
	 * Меняет позицию элемента $id на $new_position
	 * @param integer $id ID вакансии
	 * @param integer $pos Желаемая позиция в списке
	 */
	public function actionChangeposition($id, $pos)
	{
		$success = true;
		
		$model = Vacancies::model()->findByPk((int)$id);
		if ($model) {
			$model->position = $pos;
			if ($model->save())
				$success = true;
