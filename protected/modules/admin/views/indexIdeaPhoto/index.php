...
$cs->registerScript('search', "
	...
	$('.search-form form').submit(function(){
		$.fn.yiiGridView.update('index-idea-photo-grid', {
			data: $(this).serialize()
		});
		return false;
	});
...
<?php echo CHtml::link('Добавить запись', array('/admin/indexIdeaPhoto/create'), array('class' => 'primary btn')); ?>


<div class="well" style="margin-top: 10px;">
	ВНИМАНИЕ! Добавленные изображения начнут отображаться на главной странице
	только в том случае, когда активных элементов >= 6.
	Иначе показывается заглушка.
</div>


<?php $this->widget('ext.bootstrap.widgets.BootGridView', array(
	'id'           => 'index-idea-photo-grid',
	...
	'columns'      => array(
		...
		array(
			'name' => 'user_id',
			'value' => '($u = User::model()->findByPk($data->user_id)) ? $u->name : ""',
		),
		array(
			'name' => 'update_time',
