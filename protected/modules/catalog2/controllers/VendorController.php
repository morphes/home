...
class VendorController extends FrontController
{
		...
		if ($mall instanceof MallBuild) {
			...
			$viewName = '//catalog2/vendor/bm/index';
		} else {
			$viewName = '//catalog2/vendor/index';
		}
		...
		if ($mall instanceof MallBuild) {
			$this->layout = '//layouts/layoutBm';
			$this->bodyClass = 'bm-promo goods-item';
			$viewName = '//catalog2/vendor/bm/stores';
		} else {
			$viewName = '//catalog2/vendor/stores';
		}
		...
		if ($mall instanceof MallBuild) {
			$this->layout = '//layouts/layoutBm';
			$this->bodyClass = 'bm-promo goods-item';
			$viewName = '//catalog2/vendor/bm/storesInCity';
		} else {
			$viewName = '//catalog2/vendor/storesInCity';
		}
		...
		if ($uri == 'catalog2/vendor/products/id/'.$id) {
			Yii::app()->getRequest()->redirect(
				$this->createUrl('/catalog2/vendor', array_merge($_GET, array('id'=>$id, 'action'=>'products')) ),
				true,
				301
			);
		}
                ...
                if($category_id){
                    ...
                    $aCurCategory = Category::model()->findByPk( (int)$category_id );
                }
                if($collection_id) {
		...
		if ($mall instanceof MallBuild) {
			$this->layout = '//layouts/layoutBm';
			$this->bodyClass = 'bm-promo goods-item';
			$viewName = '//catalog2/vendor/bm/products';
		} else {
			$viewName = '//catalog2/vendor/products';
		}
        
        if ( isset( $model->id ) && $model->id >= 2067 && $model->id <= 3074 ){
            if ( isset( $aCurCategory ) ) {
                $this->pageTitle = $aCurCategory->name . ' ' . $model->name . ' - Купить ' . $aCurCategory->name . ' ' . $model->name . ' - цены, фото, идеи, отзывы - MyHome.ru';
                $this->description = 'Выбирайте и покупайте ' . $aCurCategory->name . ' ' . $model->name . ' на портале MyHome.ru - цены, фото, идеи, отзывы.';
                $this->keywords = $aCurCategory->name . ' ' . $model->name . ', купить, цены, фото, отзывы, идеи';
            } else {
                $this->pageTitle = 'Товары ' . $model->name . ' - Выбрать и купить товары от ' . $model->name . ' - цены, фото, идеи, отзывы - MyHome.ru';
                $this->description = 'Выбирайте и покупайте товары от ' . $model->name . '  на портале MyHome.ru - цены, фото, идеи, отзывы.';
                $this->keywords = $model->name . ', товары, купить, цены, фото, идеи, отзывы';    
            }
        } else {
            $this->pageTitle = 'Товары — ' . $model->name . ' — Производители — MyHome.ru';
        }
        ...
        {
                $store_ids = Yii::app()->dbcatalog2->createCommand()->from('cat_store_vendor')
                ...
                if($ids) {
                        $query = Yii::app()->dbcatalog2->createCommand()->selectDistinct('c.id, c.name, count(s.store_id) as qt')->from(City::model()->tableName().' c')
        ...
        {
                $categories = Yii::app()->dbcatalog2->createCommand()
                ...
                foreach($categories as $cat) {
                        ...
                        $html.=CHtml::openTag('li', array('class'=>$class))
                                . CHtml::link($cat['name'], $this->createUrl('/catalog2/vendor', array('id'=>$model->id, 'action'=>'products', 'category_id'=>$cat['id'])))
        ...
        private function getCollectionList($model, $selected_id)
        {
                ...
                foreach($collections as $collection)
                {
                        ...
                        $html.=CHtml::openTag('li', array('class'=>$class))
                                . CHtml::link($collection['name'], $this->createUrl('/catalog2/vendor', array('id'=>$model->id, 'action'=>'products', 'collection_id'=>$collection['id'])))
