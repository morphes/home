...
class StyleController extends AdminController
{
	...
	{
                $dataProvider = new CActiveDataProvider('Style', array(
                        'pagination'=>array(
                                'pageSize'=>20,
                        )));
	...
	protected function performAjaxValidation($model)
	{
        ...
        /**
         * Автокомплит опции Стиль
         * @param $term
         * @throws CHttpException
         */
        public function actionAcStyle($term)
        {
                if(!Yii::app()->request->isAjaxRequest)
                        throw new CHttpException(400);
                $this->layout = false;
                $data = Yii::app()->dbcatalog2->createCommand("SELECT t.id, t.name FROM `cat_style` t WHERE t.name LIKE '" . CHtml::encode($term) . "%'")->queryAll();
                $results = array();
