...
class ChainController extends AdminController
{
	...
	public function actionIndex()
	{
                ...
                if(isset($_POST['Chain'])) {
                        ...
                        if($model->vendor_id) {
                                $chains = Yii::app()->dbcatalog2->createCommand()
	...
	protected function performAjaxValidation($model)
	{
        ...
        public function actionAddStore()
        {
                ...
                $exists = Yii::app()->dbcatalog2->createCommand()->from('cat_chain_store')
                ...
                if(!$exists)
                        Yii::app()->dbcatalog2->createCommand()->insert('cat_chain_store', array('chain_id'=>$chain_id, 'store_id'=>$store_id));
                else
        ...
        public function actionDeleteStore($chain_id, $store_id)
        {
                ...
                Yii::app()->dbcatalog2->createCommand()->delete('cat_chain_store', 'chain_id=:cid AND store_id=:sid', array(':cid'=>$chain_id, ':sid'=>$store_id));
        }
        ...
        public function actionAddVendor()
        {
                ...
                foreach($stores as $store) {
                        ...
                        if(!$exists)
                                Yii::app()->dbcatalog2->createCommand()->insert('cat_store_vendor', array('vendor_id'=>$vendor_id, 'store_id'=>$store->id));
                        else
                                die(CJSON::encode(array('success'=>false, 'message'=>'Связка уже существует')));
                }
        ...
        public function actionAcChain($term)
        {
                if(!Yii::app()->request->isAjaxRequest)
                        throw new CHttpException(400);
                $this->layout = false;
                $data = Yii::app()->dbcatalog2->createCommand("SELECT t.id, t.name FROM `cat_chain` t WHERE t.name LIKE '" . CHtml::encode($term) . "%'")->queryAll();
                $results = array();
