...
class MainideaController extends AdminController
{
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['MainUnit'])) {
			Yii::import('idea.models.*');

			$model->attributes = $_POST['MainUnit'];
			$model->type_id = MainUnit::TYPE_IDEA;

			/** @var $origin Interior */
			$origin = $model->getOrigin();
			if ( is_null($origin) ) {
				$model->addError('origin_id', 'Оригинал не найден');
			} else {
				$photosId = $origin->getPhotosId();
				if ( !in_array($model->file_id, $photosId) ) {
					$model->addError('file_id', 'Файл не существует');
				}
			}
	...
	public function actionUpdate($id)
	{
		$id = intval($id);
		/** @var $model MainUnit */
		$model = MainUnit::model()->findByPk($id, 'type_id=:tid', array(':tid'=>MainUnit::TYPE_IDEA));

		if (is_null($model)) // Не блокируем удаленные
			throw new CHttpException(404);

		$model->setScenario('idea');
	...
	public function actionUp()
	{
		...
		if (is_null($current) || $current->type_id != MainUnit::TYPE_IDEA)
		...
		if (is_null($next) || $next->type_id != MainUnit::TYPE_IDEA)
			die ( json_encode( array('error'=>true), JSON_NUMERIC_CHECK ) );
	...
	public function actionAxGetContent()
	{
		/** @var $request CHttpRequest */
		$request = Yii::app()->getRequest();
		$id = intval( $request->getParam('idea_id') );
		if ( !$request->getIsAjaxRequest() || empty($id) )
			throw new CHttpException(404);
		...
		if ( $interior===null || !in_array($interior->status, array(Interior::STATUS_ACCEPTED, Interior::STATUS_CHANGED)) )
			throw new CHttpException(404);

		$contents = InteriorContent::model()->findAllByAttributes(array('interior_id'=>$interior->id));
		...
		foreach($contents as $content) {
			$contentNames[$content->id] = $rooms[ $content->room_id ]->option_value;
		}
