...
class StoreController extends FrontController
{
	/**
		...
		if ($mall instanceof MallBuild) {
			...
			$viewName = '//catalog2/store/bm/index';
		} else {
			$viewName = '//catalog2/store/index';
		}
	...
	public function actionListMap()
	{
		...
		$this->render('//catalog2/store/listmap', array(
			'city'        => $city,
			'cid'         => $categoryId,
	...
	public function actionList()
	{
		...
		$this->render('//catalog2/store/list', array(
	...
	private function _makeShops(&$shops, $symbols, $lang, $cityId, $category_id, $limit = 14)
	{
		if ($lang == 'spec' || $lang == 'num') {
			...
			$sql = "SELECT id, is_chain, chain_id, COUNT(*) chain_qt FROM {{store2}}"
				. " WHERE "
	...
	private function _getShopsForMap($cityId, $categoryId, $text = '')
	{
		...
		if ($result === false)
		{
			...
			while (
				...
				$stores = Yii::app()->dbcatalog2->createCommand($sql)->queryAll();
	...
	private function _getMoneyShops($cityId, $categoryId, $limit = 3)
	{
		...
		$sql = "SELECT id, product_qt FROM {{store2}}"
			. " WHERE tariff_id > :tid"
			. $this->_getCityFilter($cityId)
	...
	public function actionAjaxSearch()
	{
		...
		$html = $this->renderPartial('//catalog2/store/_storeSearch', array(
	...
	public function actionAjaxGetChain($id)
	{
		...
		$sql = 'SELECT id FROM {{store2}} WHERE chain_id = :cid AND type=:type ORDER BY id ASC LIMIT 2000';
		...
		$html = $this->renderPartial('//catalog2/store/_storeChain', array(
			'storeIds' => $storeIds,
			'chain'    => $chain,
		), true);
	...
	public function actionAjaxGetBySymbol()
	{
		...
		$html = $this->renderPartial('//catalog2/store/_storeBySymbol', array(
			'stores' => $stores
		), true);
...
//                        throw new CHttpException(403);
		...
		if ($mall instanceof MallBuild) {
			$this->layout = '//layouts/layoutBm';
			$this->bodyClass = 'bm-promo goods-item';
			$viewName = '//catalog2/store/bm/products';
		} else {
			$viewName = '//catalog2/store/products';
		}
		...
		if ($mall instanceof MallBuild) {
			$this->layout = '//layouts/layoutBm';
			$this->bodyClass = 'bm-promo goods-item';
			$viewName = '//catalog2/store/bm/feedback';
		} else {
			$viewName = '//catalog2/store/feedback';
		}
	...
	public function actionViewSite($store_id)
	{
        ...
        {
                $vendors = Yii::app()->dbcatalog2->createCommand()
	...
	public function actionMoneyIndex()
	{
		...
		$navListCategory = $this->getCategoryList($store, 0, '/catalog2/store/moneyProducts', 'moneyIndex');

		$this->render('//catalog2/store/moneyIndex', array(
			'store'           => $store,
	...
	public function actionMoneyGallery()
	{
		...
		$this->render('//catalog2/store/moneyGallery', array(
			'store'          => $store,
	...
	public function actionMoneyProducts()
	{
		...
		$navListVendor = $this->getVendorList($store, $vendor_id, '/catalog2/store/moneyProducts');
		$navListCategory = $this->getCategoryList($store, $category_id, '/catalog2/store/moneyProducts');
		 ...
		 * Рендер каталога
		 */
		$this->render('//catalog2/store/moneyProducts', array(
			'store'           => $store,
	...
	public function actionMoneyFeedback()
	{
		...
		$this->render('//catalog2/store/moneyFeedback', array(
			'store'     => $store,
			'feedback'  => $feedback,
			'feedbacks' => $feedbacks,
			'pagesize'  => $pagesize,
	...
	public function actionMoneyNews()
	{
		...
		$this->render('//catalog2/store/moneyNews', array(
			'store'        => $store,
	...
	public function actionMoneyNewsDetail($id)
	{
		...
		$this->render('//catalog2/store/moneyNewsDetail', array(
			'store' => $store,
			'news'  => $news
		));
	}
	...
	public function actionAjaxMoneyNewsEdit()
	{
		...
		$html = $this->renderPartial('//catalog2/store/_moneyNewsPopup', array(
			'store' => $store,
			'data' => $news
		), true);
	...
	public function actionAjaxMoneyNewsSave()
	{
		...
		$html = $this->renderPartial('//catalog2/store/_moneyNewsItem', array(
			'store' => $store,
			'data'  => $news
		), true);
	...
	public function actionAjaxMoneyGalleryEdit()
	{
		...
		$html = $this->renderPartial('//catalog2/store/_moneyGalleryPopup', array(
			'data' => $photo
		), true);
