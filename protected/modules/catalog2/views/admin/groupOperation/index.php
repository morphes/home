...
for ($i = 0, $ci = count($models); $i < $ci; $i++)
{
	...
	if ($model->category_id != $lastCategory) {
		$linkCategory =  CHtml::link($model->category->name, '/catalog2/admin/product/index/cid/'.$model->category_id);
	...
	$linkImg =
		'<a class="op_cart_button added_cart" title="Убрать из корзины" href="/catalog2/admin/groupOperation/dealWithCart/id/'.$model->product_id.'">
		<img src="/img/admin/small/in_cart.png" alt="Убрать из корзины">
		</a>';
	$linkProduct = CHtml::link($model->product->name, '/catalog2/admin/product/update/category_id/'.$model->category_id.'/ids/'.$model->product_id);
