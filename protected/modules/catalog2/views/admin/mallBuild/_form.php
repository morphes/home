...
<?php Yii::app()->clientScript->registerScript('mall_build', '
	...
	$("#deleteLogo").click(function(){
		...
		if(confirm("Вы хотите удалить изображение?")) {
			$.ajax({
				url: "/catalog2/admin/mallBuild/ajaxDeleteLogo",
				type: "POST",
				data: {build_id : bid},
				dataType: "JSON",
	...
	$("a.append_floor").click(function(){
		$.ajax({
			url: "/catalog2/admin/mallBuild/ajaxAppendFloor/bid/'.$model->id.'",
			type: "POST",
			dataType: "JSON",
	...
	$("div.floors").on("click", ".close", function(){
		if (confirm("Удалить этаж?")) {
			...
			$.ajax({
				url: "/catalog2/admin/mallBuild/ajaxDeleteFloor/fid/"+floor_id,
				type: "POST",
				dataType: "JSON",
				success: function(data){
	...
	$("div.floors").on("keyup", "input.floor_name", function(){
		...
		timer = setTimeout(function(){
			...
			$.ajax({
				url: "/catalog2/admin/mallBuild/ajaxUpadeNameFloor/fid/"+floor_id,
				type: "POST",
				data: {name: input.val()},
				dataType: "JSON",
				success: function(data){
