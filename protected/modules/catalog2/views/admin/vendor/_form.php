...
<?php Yii::app()->clientScript->registerScript('collections', '
	...
	disableUpDown();
        ...
        $("#coll_button_create").click(function(){
                ...
                $coll_name.val("");
                $.post("/catalog2/admin/vendor/createCollection", { vid: vid, coll_name: name }, function(response){
        ...
        $(".coll_link_delete").live("click", function(){
			...
			$.post("/catalog2/admin/vendor/deleteCollection", { vid: vid, coll_id: coll_id }, function(response){
				response = $.parseJSON(response);
				if(response.success)
        	...
        	var $this = $(this);
                ...
                timer = window.setTimeout(function(){
                	setLoader($this.parents("li"));
                        ...
                        $.post("/catalog2/admin/vendor/updateCollection", { vid: vid, coll_id: coll_id, coll_name: name }, function(response){
                                response = $.parseJSON(response);
                                if(!response.success)
                                        alert(response.message);
		...
		setLoader($li.prev("li"));
        	...
        	$.post("/catalog2/admin/vendor/changePosCollection", {vid: vid, coll_id: coll_id, direct: "up"}, function(response){
        		if (response) {
	...
	$(".coll_down:not(.disabled)").live("click", function(){
		...
		setLoader($li.next("li"));
        	...
        	$.post("/catalog2/admin/vendor/changePosCollection", {vid: vid, coll_id: coll_id, direct: "down"}, function(response){
        		if (response) {
				$li.next("li").after($li);
				disableUpDown();
        		}
