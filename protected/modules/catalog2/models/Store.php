 ...
 */
class Store extends Catalog2ActiveRecord implements IUploadImage
{
	...
	public function updateSphinx()
	{
		...
		if ($this->status == self::STATUS_DISABLED) {
			if ($this->_oldStatus != self::STATUS_DISABLED) {
				Yii::app()->gearman->appendJob('sphinx:storeDelete2', $this->id);
				$this->updateProducts();
			}
			return;
		}

		Yii::app()->gearman->appendJob('sphinx:store2', $this->id);
	...
	private function updateProducts()
	{
		...
		$data = Yii::app()->dbcatalog2->createCommand($sql)->bindParam(':sid', $sid)
	...
	public function rules()
	{
		return array(
			...
			array('image_id, xml_parser_id, city_id, user_id, admin_id, subdomain_id, tariff_id, tariff_id_new, tariff_expire_date, mall_build_id, floor_id, bindedToProduct', 'numerical', 'integerOnly'=>true),
			...
			array('address, time, xml_url', 'length', 'max'=>1000),
...
//                                'pattern'=>'/^(http(s?)\:\/\/)?(([0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя][0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя_-]*)(\.[0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя][0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя_-]*)+(\/[0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя][0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя_-]*)*(\/?(\?([0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя][-0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя_\[\]]*(=[-0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя_\[\]\,\'\\\+%\$#]*){0,1}(&[0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя][-0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя_\[\]]*(=[-0-9a-zA-ZА-Яабвгдеёжзийклмнопрстуфхцчшщъыьэюя_\[\]\,\'\\\+%\$#]*){0,1})*){0,1})?))$/i',
			...
			array('id, type, xml_url, contractor_id, bindedToProduct, mall_build_id, floor_id, sect_name, tariff_id, tariff_expire_date, tariff_expire, image_id, city_id, name, activity, email, site, admin_id, phone, address, geocode, time, about, bg_class, create_time, update_time, xml_parser_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'type'		  => 'Тип магазина',
            'anchor'      => "Анкор ссылки",
            'xml_url'         => 'Ссылка на XML файл товаров',
            'xml_parser_id'   => 'Парсер XML файла',
		);
	}
	...
	public function search($limit = null, $productId = null)
	{
        ...
        {
                if (!$byStoreVendor)
                        $vendors_list = Yii::app()->dbcatalog2->createCommand()
        ...
        {
                $chain_id = Yii::app()->dbcatalog2->createCommand()->select('chain_id')->from('cat_chain_store')
		...
		if ( $this->type == self::TYPE_ONLINE )
			return null;
                ...
                $html = '';
                $html.= CHtml::link($this->name, Yii::app()->createUrl('/catalog2/store/index', array('id'=>$this->id)));
	...
	public function mallCheck($attribute, $params)
	{
        ...
        {
                if(!$this->_moderators) {
                        $moderators = Yii::app()->dbcatalog2->createCommand()->select('moderator_id')
        ...
        {
                $sql = "SELECT product_qt FROM {{store2}} WHERE id = :id";
		$id = intval($this->id);
				...
				case 'moneyAbout':
					if ($store->tariff_id == Store::TARIF_MINI_SITE) {
						$url = Yii::app()->createAbsoluteUrl(
							'catalog2/store/moneyIndex',
							array('sub' => $store->subdomain->domain)
						);
					} else {
						$url = Yii::app()->createUrl(
							'catalog2/store/index',
							array('id' => $store->id)
						);
					}
					break;
	...
	{
		if ($useSphinx) {
			$sql = "SELECT category_ids FROM {{store2}} WHERE  id=:storeId LIMIT 10";
			$storeId = (int)$this->id;
			...
			$useCategories = Yii::app()->dbcatalog2->createCommand()->selectDistinct('p.category_id')
        ...
        {
                if (!$this->_owners) {
                        $this->_owners = Yii::app()->dbcatalog2->createCommand()->select('moderator_id')
                                ->from('cat_store_moderator')
                                ->where('store_id=:sid', array(':sid'=>$this->id))->queryColumn();
                        if ($this->admin_id)
	...
	public function unbindVendorFromCatStorePrice($vendorId)
	{
		...
		Yii::app()->dbcatalog2->createCommand($sql)->bindValues(array(
			':sid' => $this->id,
			':vid' => $vendorId
		))->execute();
	}
	...
	public static function getRelatedShops($categoryId, $cityId, $explodeIdStore, $limit = 6)
	{
		...
		if (!empty($storesList)) {
			...
			$sql = "SELECT id FROM {{store2}} WHERE category_ids=:catId AND id IN (:stores) AND id!=:storeId  AND tariff_id>:tariffId ORDER by rand() LIMIT :limit";
		...
		if ($countPaidShops < $limit && !empty($storesList)) {
			$sql = "SELECT id FROM {{store2}} WHERE category_ids=:catId AND id IN (:stores) AND id!=:storeId AND tariff_id=:freeTariff GROUP BY chain_id LIMIT 100";
			$sql = str_replace(':stores', $storesStr, $sql);
    ...
    public static function getStoreInCity($ids, $city_id) {
        ...
        $rez = Yii::app()->dbcatalog2->createCommand($sql)
