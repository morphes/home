 ...
 */
class StatStore extends Catalog2ActiveRecord
{
	...
	static public function updateStatStoreMySql($pattern)
	{
		...
		try {
			foreach ($keys as $key) {
				...
				$selectResult = Yii::app()->dbcatalog2->createCommand($sqlSelect)
					->bindParam(':time', $time)
				...
				if (!$selectResult) {
					Yii::app()->dbcatalog2->createCommand($sqlInsert)
						->bindParam(':store_id', $store_id)
						->bindParam(':type', $type)
						->bindParam(':view', $view)
						->bindParam(':time', $time)
					...
					Yii::app()->dbcatalog2->createCommand($sqlUpdate)
						->bindParam(':id', $id)
						->bindParam(':view', $view)
						->execute();
				}
