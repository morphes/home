...
class ForumAnswerController extends AdminController
{
	...
	{
		$topic = ForumTopic::model()->findByPk((int)$topic_id);
		if ( ! $topic)
			throw new CHttpException(500, 'Error topic ID');
	...
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			$model = $this->loadModel($id);
			$model->status = ForumAnswer::STATUS_DELETED;
			$model->save(false);
	...
	{
		$model=new ForumAnswer('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ForumAnswer']))
			$model->attributes=$_GET['ForumAnswer'];
	...
	{
		$model=ForumAnswer::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='forum-answer-form')
		{
