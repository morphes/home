 ...
 */
class Product extends EActiveRecord implements IUploadImage, IComment
{
	...
	public function saveUsagePlace()
	{
		...
		Yii::app()->db->createCommand($sql)->bindParam(':pid', $pid)->execute();
	...
	public function rules()
	{
		...
		return array(
			...
			array('barcode, name, guaranty', 'length', 'max' => 255),
			...
			array('id, contractor, category_id, status, vendor_id, collection_id, country, guaranty, related_product, usageplace, eco, user_id, image_id, price, barcode, name, tags, desc, admin_comment, create_time, update_time', 'safe', 'on' => 'search'),
		);
	}
	...
	public function getUsageplace()
	{
		...
		$pid = $this->id;
		$this->_usagespace = Yii::app()->db->createCommand($sql)->bindParam(':pid', $pid)->queryColumn();
	...
	public function getImagePath()
	{
		...
		switch ($this->_imageType) {
			case 'product':
				return 'catalog' . '/' . intval($this->user_id / UploadedFile::PATH_SIZE + 1) . '/' . $this->user_id . '/product/' . $this->id;
			default:
	...
	public function getMinicardOptions()
	{
		 ...
		 */
		$options = Yii::app()->db->createCommand()->select('id, name, type_id, param')->from('cat_option')
		 ...
		 */
		$values = Yii::app()->db->createCommand()->select('id, option_id, value')->from('cat_value')
	...
	public function getFullcardOptions()
	{
		 ...
		 */
		$options = Yii::app()->db->createCommand()->select('id, name, group_id, type_id, param')->from('cat_option')
	...
	static public function getLink($id, $store_id=null, $category_id=null)
	{
		...
		return Yii::app()->createUrl('/product', $params);
	}
	...
	{
		return Yii::app()->db->createCommand()->select('count(id)')->from(Feedback::model()->tableName())
	...
	{
		$sql = 'SELECT @count FROM {{product}} WHERE mall_ids = '.$mall->id.' GROUP BY mall_ids';
	...
	static public function getRandomProducts($size = 6)
	{
		...
		$sql = "SELECT MAX(id) as max_id, category_id FROM {{product}} WHERE price >= 0.001 GROUP BY category_id LIMIT 100";
		...
		foreach($cats as $cat)
		{
			...
			$sql = 'SELECT id FROM {{product}} WHERE price >= 0.001 AND category_id = '.$cat['category_id'].' AND id > '.$rnd.' LIMIT 1';
	...
	static public function getRandomProductsMall($size = 6)
	{
		...
		// Получаем список всех доспуных категорий и маскимально доступный ID товара в каждой из них.
		$sql = 'SELECT MAX(id) as max_id, category_id FROM {{product}} WHERE price >= 0.001 AND mall_ids = '.$mall->id.' GROUP BY category_id LIMIT 100';
		$cats = Yii::app()->sphinx->createCommand($sql)->queryAll();
		...
		foreach($cats as $cat)
		{
			// Получаем случайный товар
			$rnd = rand(0, $cat['max_id'] - 1);
			$sql = 'SELECT id FROM {{product}} WHERE price >= 0.001 AND category_id = '.$cat['category_id'].' AND id > '.$rnd.' AND mall_ids = '.$mall->id.' LIMIT 1';
			$p = Yii::app()->sphinx->createCommand($sql)->queryScalar();
	...
	public function getStoresIds()
	{
		if($this->_storesIds === null)
			$this->_storesIds = Yii::app()->db->createCommand()->select('store_id')->from('cat_store_price')
	...
	public static function countAveragePrice($pid)
	{
		...
		$avgPrice = Yii::app()->db->createCommand("SELECT AVG(price) as avg_price FROM cat_store_price WHERE product_id = :pid AND price > 0")->bindValue(':pid', $model->id)->queryScalar();
...
//
//			$categoryList = Yii::app()->db->createCommand($sql)->bindParam(':lft', $lft)
...
//	}
	...
	{
		$sql = "SELECT (id) FROM {{product}} WHERE category_id=:catId AND store_ids=:storeId AND id!=:productId ORDER by rand() LIMIT :limit";
	...
	{
		Yii::import('catalog.models.StoreGeo');
		$cityId = (int)$cityId;
		$limit = (int)$limit;
	...
	static public function getRandomProductsIds($size = 6, $categories = array())
	{
		...
		if ($categories) {
			...
			$sql = 'SELECT id FROM {{product}} WHERE price >= 0.001 and category_id IN (' . $categoriesList . ') and category_id <> 22 ORDER BY RAND() LIMIT ' . $size;
		} else {
			$sql = 'SELECT id FROM {{product}} WHERE price >= 0.001 ORDER BY RAND() LIMIT ' . $size;
		}
    ...
    public function getSimilarNew($city_id=null)
    {
        ...
        $productIds = Yii::app()->db->createCommand('select distinct p.id from cat_store_price sp
