 ...
 * @package Catalog
 */
class Category extends EActiveRecord implements IUploadImage
{
	...
	{
		Yii::app()->cache->delete('CATEGORY:ENGNAMES');
	}
	...
	public function search()
	{
        ...
        {
                $result = array();
                $categories = Yii::app()->db->createCommand('SELECT t.id, t.name FROM cat_category t WHERE (t.rgt - t.lft) = 1 ORDER BY t.level')->queryAll();
	...
	static public function getCategoriesWithActive($category = null, $city = null)
	{
        ...
        {
                $descendants = Yii::app()->db->createCommand()->select('id')->from('cat_category c')
        ...
        {
                return Yii::app()->createUrl('/catalog/category/list', array('id'=>$id));
        }
	...
	public function getLastDescendants($order_field = 'lft')
        {
                $descendants = Yii::app()->db->createCommand()->select('id, name, product_qt')->from('cat_category c')
	...
	static public function setMaxPrice($cid = null)
	{
		...
		if ($model) {
			$max_price = Yii::app()->db->createCommand("SELECT MAX(average_price) as max_price FROM cat_product WHERE category_id = '{$cid}'")->queryScalar();
	...
	public function getMaxPrice()
	{
		...
		$sql = 'SELECT MAX(price) FROM {{product}} WHERE category_id IN (' . implode(',', $ids) . ')' . $condition . ' GROUP BY category_id';
	...
	public static function getNotEmptyByCity($cityId)
	{
		...
		if (!empty($storeList)) {
			...
			$sql = 'SELECT COUNT(*) as cnt, category_id FROM {{product}} WHERE store_ids IN ('.$storeList.') GROUP BY category_id LIMIT 100';
	...
	public static function getNotEmptyByMall($mallId)
	{
		...
		$sql = 'SELECT COUNT(*) as cnt, category_id FROM {{product}} WHERE mall_ids=:mid GROUP BY category_id LIMIT 100';
	...
	public static function getCacheKeyCategoryMall($mallId, $categoryId)
	{
		...
		return 'CATALOG:MALL:' . $mallId . ':CATEGORY:' . $categoryId;
	}
	...
	public static function getCategoryName($id)
	{
		if ( self::$_categoryData === false ) {
			$data = Yii::app()->cache->get('CATEGORY:ENGNAMES');
			if ( $data === false ) { // получение из БД
				$sql = 'SELECT id, eng_name FROM cat_category';
				$data = Yii::app()->db->createCommand($sql)->setFetchMode(PDO::FETCH_KEY_PAIR)->queryAll();
				Yii::app()->cache->set('CATEGORY:ENGNAMES', $data, Cache::DURATION_DAY);
			}
