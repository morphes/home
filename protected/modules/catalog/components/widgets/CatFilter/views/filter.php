...
echo CHtml::beginForm(Yii::app()->createUrl('/catalog/category/list', $urlParams), 'get', array(
        'id'=>'filter_form'
)); ?>
        ...
        foreach($options as $option)
        {
                ...
                switch($option['type_id']) {
                        ...
                        case Option::TYPE_SELECT :
                                $values = Yii::app()->db->createCommand()->from('cat_value')->order('position')
                        ...
                        case Option::TYPE_COLOR :
                                $values = Yii::app()->db->createCommand()->from('cat_color')->queryAll();
                        ...
                        case Option::TYPE_STYLE :
                                $values = Yii::app()->db->createCommand()->from('cat_style')->queryAll();
                                $dropdownValues = array();
...
<?php Yii::app()->clientScript->registerScript('filter', '
        $("#vendor_country").change(function(){
                ...
                $.post("/catalog/category/ajaxVendorsList", {country_id: cid, category_id:category_id}, function(response){
