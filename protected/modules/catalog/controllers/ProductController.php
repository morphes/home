...
class ProductController extends FrontController
{
	public function beforeAction($action)
	{
		...
		Yii::app()->clientScript->registerScriptFile('/js-new/catalog.js');
	...
	public function actionIndex($id, $cid = false)
	{
		...
		$viewName = '//catalog/product/index';
		$this->bodyClass = 'goods goods-item';
	...
	public function actionIndexOld($id)
	{
		 ...
		 * Рендеринг карточки товара
		 */
		$this->render('//catalog/product/bmIndex', array(
			'model'            => $model,
			'feedbacks'        => $feedbacks,
			'city'             => $city,
			'storesInUserCity' => $storesInUserCity,
	...
	public function actionDescription($id)
	{
		...
		if ($mall instanceof MallBuild) {
			$this->bodyClass = 'bm-promo goods-item';
			$this->layout = '//layouts/layoutBm';
			$viewName = '//catalog/product/bmDescription';
	...
	public function actionComment($id)
	{
		...
		if ($mall = Cache::getInstance()->mallBuild) {
			...
			$viewName = '//catalog/product/bmComment';
		} else {
			//Редирект на новую карточку товара
			$this->redirect($model->getElementLink(), true, 301);
		}
	...
	public function actionFeedback($id)
	{
		...
		if ($mall = Cache::getInstance()->mallBuild) {
			...
			$viewName = '//catalog/product/bmFeedback';
		} else {
			//Редирект на новую карточку товара
			$this->redirect($model->getElementLink(), true, 301);
		}
	...
	public function actionCreateFeedback($id)
	{
		...
		if (isset($_POST['Feedback']) && !$model->checkFeedback) {
			...
			if ($feedback->save()) {
				$html = $this->renderPartial('//catalog/product/_feedbackItemIndexPageGrid', array('data' => $feedback), true);
	...
	{
		$userStoresName = Yii::app()->db->createCommand()->select('name')
			->from('cat_store s')
	...
	public function actionStores($id)
	{
		...
		if ($mall = Cache::getInstance()->mallBuild) {
			...
			$viewName = '//catalog/product/bmStores';
		} else {
			//Редирект на новую карточку товара
			$this->redirect($model->getElementLink(), true, 301);
		}
	...
	public function actionStoresInCity($id, $cid)
	{
		...
		if ($mall = Cache::getInstance()->mallBuild) {
			...
			$viewName = '//catalog/product/bmStoresInCity';
		} else {
			//Редирект на новую карточку товара
			$this->redirect($model->getElementLink(), true, 301);
		}
	...
	private function getStoreCities($model)
	{
		...
		$ids = Yii::app()->db->createCommand()->from('cat_store_price c')
	...
	public function actionGetGalleryAjax()
	{
		...
		$html = $this->renderPartial('//catalog/product/_gallery',array('model' => $model), true);
