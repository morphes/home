...
class CategoryController extends FrontController
{
	...
	public function actionList($id = 0)
	{
		...
		if ($mall && $dataProvider->getTotalItemCount() == 0) {
			$htmlEmpty = '<div class="-col-9"><p class="-large -gutter-top">'
				.'К сожалению, в нашем каталоге еще нет товаров из ТВК «Большая Медведица».<br>
				      Рекомендуем вам посмотреть <a onclick="CCommon.setUrl(\'catalog\'); ">каталог всех товаров</a>, представленных на MyHome'
				.'</p></div>';
		}
		...
		if ($mall)
		{
			...
			$viewName = '//catalog/category/bmListGrid';
			...
			$viewName = '//catalog/category/listGrid';
		}
	...
	private function _getSortStringForProvider()
	{
		...
		$sort = $cook_sort ? $cook_sort->value : 'default';
		$sort = explode('_', $sort);
		...
		switch ($sort[0]) {
			case 'date':
				...
				break;
			case 'price':
				$sortString = 'sort_price ASC, price '.$order;
				break;
			case  'default':
...
//		}
	...
	private function _getOptionsForProvider(&$selected, $category)
	{
		...
		if ( !is_null($selected['style']) ) {
			$style = Yii::app()->db->createCommand()->select('name')
	...
	private function _getHtmlForEmptyCat($category, $city, $mall)
	{
		...
		$cacheKey = 'NOTFOUND_PRODUCTS:' . $category->id . ':' . $city->region_id;
		...
		if (!is_array($cities) || empty($cities) ) {
			...
			/** @var $cityItem City */
			$presql = 'SELECT COUNT(*) as qt FROM {{product}}'
