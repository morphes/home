...
class ProfileController extends FrontController
{
        ...
        public function actionProductUpdate($id)
	{
        ...
        public function actionProductUploadImage()
        {
                ...
                $model->setImageType('product');
                $file = UploadedFile::loadImage($model, 'file', '', false, null, true);
        ...
        public function actionProductDeleteImage()
        {
                ...
                if($model->image_id == $file->id) {
                        ...
                        $new_image_id = Yii::app()->db->createCommand()->select('file_id')->from('cat_product_image')
		...
		$criteria->group = 't.id';
                ...
                $this->render('//catalog/profile/store_list', array(
                        'dataProvider'=>$dataProvider,
                ), false, array('profileStore', array('user' => Yii::app()->user->model)));
        }
	...
	public function actionStoreStat($id)
	{
		...
		$this->render('//catalog/profile/store_stat', array(
			'store' => $store,
			'stat'  => $stat
		), false, array('profileStore', array('user' => Yii::app()->user->model)));
	}
                        ...
                        */
                        if (!$errors && $model->save()) {
                                if (!$from_card)
                                        ...
                                        $this->redirect($this->createUrl('/catalog/store/index', array('id'=>$model->id)));
                        }
	...
	public function actionFolderList()
	{
		...
		$this->render('//catalog/profile/folder_list', array('items' => $items), false, array(
			'profileMall', array(
