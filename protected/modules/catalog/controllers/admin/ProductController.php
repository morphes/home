...
class ProductController extends AdminController
{
	...
	public function actionImageUrlUpload($pid)
	{
        ...
        public function actionCreateSimilar()
        {
                 ...
                 */
                $spid_similars = Yii::app()->db->createCommand()
        ...
        {
                $exist = Yii::app()->db->createCommand()->select('count(product_id)')
                        ->from('cat_similar_product')
        ...
        {
                $result = Yii::app()->db->createCommand()->insert('cat_similar_product', array(
                        'product_id'=>$pid,
        ...
        public function actionDeleteSimilar()
        {
                ...
                // удаление связки
                $success = Yii::app()->db->createCommand()->delete('cat_similar_product',
        ...
        public function actionCountry($term)
        {
                if(!Yii::app()->request->isAjaxRequest)
                        throw new CHttpException(400);
                $this->layout = false;
                $data = Yii::app()->db->createCommand("SELECT t.id, t.name FROM `country` t WHERE t.name LIKE '" . CHtml::encode($term) . "%'")->queryAll();
                $results = array();
	...
	public function actionGetStatProduct()
	{
		...
		$command->join = 'LEFT JOIN user ON user.id = t.user_id';
