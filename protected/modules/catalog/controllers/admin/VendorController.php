...
class VendorController extends AdminController
{
	...
	protected function performAjaxValidation($model)
	{
		...
		$maxPos = Yii::app()->db->createCommand("SELECT MAX(position) FROM cat_vendor_collection WHERE vendor_id = ".$vendor->id)->queryScalar();
		...
		$collectionId = Yii::app()->db->lastInsertID;
		...
		if ($collection)
		{
			...
			Yii::app()->db->createCommand("SET @num := 0; UPDATE cat_vendor_collection SET position = (@num := @num + 1) WHERE vendor_id = {$vendor->id} ORDER BY position ASC ")->execute();
		...
		switch($direct)
		{
			case 'up':
				...
				if ($newPos >= 1) {
					...
					Yii::app()->db->createCommand("UPDATE cat_vendor_collection SET position = position + 1 WHERE vendor_id = '".$vendor->id."' AND position = '".$newPos."'")->execute();
					// Текущему элементу ставим новую позицию
					Yii::app()->db->createCommand("UPDATE cat_vendor_collection SET position = '".$newPos."' WHERE id = '".$collection['id']."'")->execute();
				}
				break;
			case 'down':
				$newPos = $collection['position'] + 1;
	 ...
	 * TODO: FIX THIS SHIT!!! city_name index
        ...
        public function actionAutocompleteCountry($term = null)
        {
                ...
                $countries = Yii::app()->db->createCommand()->from(Country::model()->tableName())
