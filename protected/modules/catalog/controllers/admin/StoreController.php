...
//		$model->deleteSphinx();
	...
	public function actionIndex()
	{
        ...
        if (isset($_GET['Store'])) {
            ...
            if ($_GET['Store']['category']) {
                $sql = "SELECT id FROM {{store}} WHERE category_ids = :category_id LIMIT 10000";
					...
					if (($handle = fopen($basePath.$folder.'/'.$fileName, "r")) !== false)
					{
						...
						if (CatCsv::checkImportCsvHead($data, CatCsv::TYPE_STORE))
						{
							...
							if ($taskImport->save())
							{
								...
								$this->redirect('/catalog/admin/catCsv/list');
							} else {
	...
	protected function performAjaxValidation($model)
	{
        ...
        public function actionAddVendor()
        {
                ...
                if(!$exists) {
                        Yii::app()->db->createCommand()->insert('cat_store_vendor', array('vendor_id'=>$vendor_id, 'store_id'=>$store_id));
		...
		$store = Store::model()->findByAttributes(array('id' => $storeId));
                ...
                Yii::app()->db->createCommand()->delete('cat_store_vendor', 'store_id=:sid AND vendor_id=:vid', array(':vid'=>$vendor_id, ':sid'=>$store_id));
        ...
        public function actionClone($id)
        {
                ...
                $vendors_ids = Yii::app()->db->createCommand()->select('vendor_id')->from('cat_store_vendor')
                        ->where('store_id=:id', array(':id'=>$source->id))->queryAll();

                foreach($vendors_ids as $v) {
                        Yii::app()->db->createCommand()->insert('cat_store_vendor', array(
                                'store_id'=>$target->id,
        ...
        public function actionDeleteModerator($store_id, $moderator_id)
        {
                ...
                Yii::app()->db->createCommand()->delete('cat_store_moderator', 'store_id=:sid AND moderator_id=:vid', array(':vid'=>$moderator_id, ':sid'=>$store_id));
	...
	public function actionAjaxDeleteStoreGeo()
	{
		...
		$deleteResult = Yii::app()->db->createCommand()
