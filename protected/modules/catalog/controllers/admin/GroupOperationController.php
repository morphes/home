...
class GroupOperationController extends AdminController
{
		...
		foreach ($models as $model) {
			...
			foreach ($models as $model2) {
				if ($product_id != $model2->product_id) {
					if ( ! Yii::app()->db->createCommand("SELECT product_id FROM cat_similar_product WHERE product_id = '".$product_id."' AND similar_product_id = '".$model2->product_id."'")->queryScalar()) {
						Yii::app()->db->createCommand("INSERT INTO cat_similar_product (`product_id`, `similar_product_id`) VALUES ('".$product_id."', '".$model2->product_id."')")->execute();
					}
				}
			}
		}
	...
	private function makeRedirect($category_id)
	{
		...
		if ( ! empty($ids))
			$this->redirect('/catalog/admin/product/update/category_id/'.$category_id.'/ids/'.implode(',', $ids));
	}
}