...
class ChainController extends FrontController
{
        ...
        public function actionIndex($id)
        {
                ...
                $this->render('//catalog/chain/index', array(
                        'model'=>$model,
                        'cities'=>$cities,
                ));
        }
        ...
        public function actionStores($id)
        {
                ...
                $this->render('//catalog/chain/stores', array(
                        'model'=>$model,
                        'sorted'=>$sorted,
                ));
        }
        ...
        public function actionStoresInCity($id, $cid)
        {
                ...
                $this->render('//catalog/chain/storesInCity', array(
                        'model'=>$model,
                        'stores'=>$stores,
                        'cities'=>$cities,
                        'city'=>$city,
        ...
        {
                $store_ids = Yii::app()->db->createCommand()->from('cat_chain_store')
                ...
                if($ids)
                        $cities = Yii::app()->db->createCommand()->select('c.id, c.name, count(s.store_id) as qt')->from(City::model()->tableName().' c')
