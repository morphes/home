...
<!-- Модальное окно установки цен на товар в магазинах (содержимое обновляется аяксом при открытии окна) -->
	...
	<?php Yii::app()->getClientScript()->registerScript('price-modal', '
		...
		$("#product-form").on("click", ".price", function(){
			...
			$.post("/catalog/admin/product/ajaxProductStoreBind/pid/"+pid, {}, function(response){
...
<?php Yii::app()->clientScript->registerScript('initial', '
        ...
        $(".clone").live("click", function(){
                ...
                $.post("/catalog/admin/product/productClone/id/" + pid + "/category_id/' . $category->id . '", formDataForSave, function(response) {
                        if(response.result)
        ...
        $(".delete").live("click", function(){
                ...
                if (confirm("Вы действительно хотите удалить товар?")) {
                        ...
                        $.post("/catalog/admin/product/delete/id/" + pid, null, function(response) {
                                if(response.result)
                                        $("#product-" + pid).remove();
        ...
        $(".deleteFile").live("click", function(){
                ...
                $.post("/catalog/admin/product/deleteFile", {type: type, file_id: file_id, pid: pid, vid:vid}, function(response) {
                        if(response.result)
        ...
        $(".similar-button-add").live("click", function(){
                ...
                $.post("/catalog/admin/product/createSimilar", {pid: pid, spid: spid}, function(response) {
                        if(response.success)
        ...
        $(".similar-button-delete").live("click", function(){
                ...
                if (confirm("Вы действительно хотите удалить связку с аналогичным товаром?")) {
                        $.post("/catalog/admin/product/deleteSimilar", {pid: pid, spid: spid}, function(response) {
                                if(response.success)
        ...
        $(".color-value-add").live("click", function(){
                ...
                $.post("/catalog/admin/product/colorValue", {action: "add", value_id: value_id, value: value}, function(response) {
                        if(response.success)
        ...
        $(".color-value-delete").live("click", function(){
                ...
                $this = $(this);
                $.post("/catalog/admin/product/colorValue", {action: "delete", value_id: value_id, value: value}, function(response) {
                        if(response.success)
                                $this.parent().remove();
                        else
                                alert(response.message);
...
<?php Yii::app()->clientScript->registerScript('autoSave', '
        ...
        function autoSaveForm() {
                ...
                if(pidsForSave.length > 0) {
                        ...
                        $.post("/catalog/admin/product/update/ids/" + pidsForSave + "/category_id/" + cid, formDataForSave, function(response) {
...
<script type="text/javascript">
	$(function(){
		...
		$('#product-form').on('click', 'a.crop_img', function(){
			var data = $(this).data();
			var w = window.open('/catalog/admin/product/cropCover/type/'+data['type']+'/pid/'+data['pid']+'/fileid/'+data['fileid'], '', 'top=10,left=0,width=600,height=600,location=0,menubar=0');
