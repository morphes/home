<?php
        ...
        $this->breadcrumbs=array(
                ...
                $rootParent->name=>array('/catalog/admin/category/index', 'cid'=>$rootParent->id),
                'Список',
        );
...
<div>
        ...
        <?php echo CHtml::button('Добавить товары', array('class'=>'primary btn','style'=>'float:right; margin-left: 10px;', 'onclick'=>'document.location = \''.$this->createUrl('/catalog/admin/product/create/', array('category_id'=>$category->id)).'\''))?>
        <?php echo !($category->productExists) ? CHtml::button('Добавить подкатегорию', array('class'=>'primary btn','style'=>'float:right;', 'onclick'=>'document.location = \''.$this->createUrl('/catalog/admin/category/create/in/' . $category->id).'\'')) : ''; ?>
