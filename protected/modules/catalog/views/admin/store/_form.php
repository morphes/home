		...
		<div class="input">
			...
			<script>
				...
				$('#add-image').on('click', '.head_image_delete', function(){
					$.ajax({
						url: '/catalog/admin/store/headImageDelete',
						data: {
				...
				<?php Yii::app()->getClientScript()->registerScript('geo', '
					...
					$("#store_geo_type").change(function(){
						...
						if (selected_geo_type=="cng") {
							if (confirm("Добавить все страны СНГ?")) {
								$.ajax({
									url: "/catalog/admin/store/ajaxSetCountries",
									type: "POST",
									async: false,
									dataType: "json",
					...
					$("#store_geo_id").change(function(){
						...
						$.ajax({
							...
							data: data,
							type: "post",
							url: "/catalog/admin/store/ajaxCreateStoreGeo",
							success: function(response){
								if (!response.success) {
									alert(response.error);
									return false;
					...
					$("#online_store_geo_saved").on("click", ".deleteGeo", function(){
						...
						$.ajax({
							...
							data: data,
							type: "post",
							url: "/catalog/admin/store/ajaxDeleteStoreGeo",
							success: function(response){
								if (!response.success) {
									alert(response.error);
									return false;
		...
		<script type="text/javascript">
			...
			$('.select_mall').click(function(){
				...
				if ($(this).is(':checked')) {
					...
					$.ajax({
						url: '/catalog/admin/mallBuild/ajaxGetFloorsById/id/'+build_id,
						type: 'POST',
						dataType: 'JSON',
						success: function(data){
        ...
        <?php Yii::app()->clientScript->registerScript('submit-form', '
        	...
        	$("#Store_type").change(function(){
				...
				mall_select: $("input.select_mall").is(":checked")
                        ...
                        $.post("/catalog/admin/store/findDublicates", data, function(response){
                ...
                <?php $this->widget('ext.bootstrap.widgets.BootGridView',array(
                                ...
                                'columns'=>array(
                                        ...
                                        array(
                                                ...
                                                'deleteButtonUrl'=>'"/catalog/admin/store/deleteVendor/store_id/'.$model->id.'/vendor_id/".$data->id'
                                        ),
                                ),
                        )); ?>
                ...
                <?php Yii::app()->clientScript->registerScript('vendor', '
                        ...
                        $("#deleteImage").click(function(){
                                ...
                                if(confirm("Вы хотите удалить изображение?")) {
                                        $.post("/catalog/admin/store/deleteImage", {store_id: sid}, function(response) {
                                        if(response.success)
                                                $(".storeImage").remove();
                                        else
                        ...
                        $("#button-add-vendor").click(function(){
                                ...
                                $.post("/catalog/admin/store/addVendor", {store_id: sid, vendor_id: vid}, function(response) {
                                        if(response.success)
        ...
        <?php if(!$model->isNewRecord) : ?>
                ...
                <?php $this->widget('ext.bootstrap.widgets.BootGridView',array(
                        ...
                        'columns'=>array(
                                ...
                                array(
                                        ...
                                        'deleteButtonUrl'=>'"/catalog/admin/store/deleteModerator/store_id/'.$model->id.'/moderator_id/".$data->id'
                                ),
                        ),
                )); ?>
                ...
                <?php Yii::app()->clientScript->registerScript('moderator', '
                        $("#button-add-moderator").click(function(){
                                ...
                                $.post("/catalog/admin/store/addModerator", {store_id: sid, moderator_id: mid}, function(response) {
                                        if(response.success)
