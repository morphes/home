...
<div class="row form-stacked">
	<div class="span3">
		<div class="actions">
			<?php echo CHtml::link('Добавить товар', '/catalog/admin/store/addGood/id/'.$store->id, array('class' => 'btn'));?>
		</div>
	</div>
...
<?php $this->widget('ext.bootstrap.widgets.BootGridView', array(
	...
	'columns'        => array(
		...
		array(
			...
			'value' => 'CHtml::link($data->product->name, $data->product->getLink($data->product_id))',
			'type'  => 'raw'
		),
		array(
			'name'  => 'Категория',
		...
		array(
			'name'  => 'Производитель',
			'value' => '$data->product->vendor->name',
			'type'  => 'raw'
		),
		array(
			'name'  => 'price',
		...
		array(
			'buttons'  => array(
				'delete' => array(
					'label' => 'Клон',
					'url'   => 'Yii::app()->createUrl("/catalog/admin/store/deletePrice/", array("sid" => $data->store_id, "pid" => $data->product_id))',
				)
			),
			'class'    => 'CButtonColumn',
			'template' => '{delete}',
...
<script type="text/javascript">
	function removeGoods(){
		...
		if (confirm("Вы действительно хотите удалить выбранные товары?")) {
			$.post("/catalog/admin/store/deletePrice/sid/<?php echo $store->id;?>/ids/"+products, function(response) {
	...
	$(function(){
		...
		$body.on({
			'focusout':function(){
				...
				$.post(
					'/catalog/admin/store/updatePrice/sid/'+sid+'/pid/'+pid,
					{ price: price },
					function(response){
						if ( ! response.success) {
		...
		$body.on({
			'focusout':function(){
				...
				$input.next('img').css('visibility', 'visible');
				$.post(
					'/catalog/admin/store/updateDiscount/sid/'+sid+'/pid/'+pid,
					{ discount: discount },
					function(response){
						if ( ! response.success) {
							alert(response.errorMsg);
