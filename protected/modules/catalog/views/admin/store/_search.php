...
<div class="row">
		...
		$authors = Yii::app()->db->createCommand("SELECT DISTINCT user.id as id, CONCAT(user.lastname, ' ', user.firstname) as name FROM user"
			."  INNER JOIN cat_store ON cat_store.user_id = user.id ORDER BY user.lastname ASC"
		...
		<div class="clearfix">
			...
			<div class="input">
                ...
                <div class="clearfix">
                    ...
                    <div class="input">
                        <?php
                       $category = Yii::app()->db->createCommand()->select('name')->from('cat_category')->where('id=:id', array(':id'=>(int)$category_id))->limit(1)->queryScalar();
                        $this->widget('application.components.widgets.CAjaxAutoComplete', array(
                            'name'=>'Category',
                            'value'=>$category,
                            'sourceUrl'=>'/catalog/admin/category/acCategory',
                            'options'=>array(
                                'minLength'=>'2',
                                'showAnim'=>'fold',
