...
<div>
        <?php echo !empty($rootParent) ? CHtml::button("Вверх ↑ ", array('class'=>'btn','style'=>'float:left', 'onclick'=>'document.location = \''.$this->createUrl('/catalog/admin/category/index/cid/' . @$rootParent->id).'\'')) : '';?>

        <?php echo CHtml::button('Добавить подкатегорию', array('class'=>'primary btn','style'=>'float:right', 'onclick'=>'document.location = \''.$this->createUrl('/catalog/admin/category/create/in/' . $root->id).'\''))?>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('form.form_stat').submit(function(){
			...
			$.get(
				'/catalog/admin/product/getStatProduct',
				{
...
<?php $this->widget('ext.bootstrap.widgets.BootGridView',array(
	...
	'columns'=>array(
		array(
                        'name'=>'Категория',
                        'type'=>'raw',
                        'value'=>'CHtml::link($data->name, "/catalog/admin/category/index/cid/" . $data->id)',
                ),
                array(
                        'name'=>'Подкатегорий',
                        'type'=>'raw',
		...
		array(
			'class'=>'CButtonColumn',
				...
				'settings' => array(
					...
					'url'=>'Yii::app()->createUrl("/catalog/admin/category/settings/", array("id"=>$data->id))',
					'imageUrl'=>'/img/admin/small/settings.png',
				),
                                'sort' => array(
                                        'label'=>'сорт.',
                                        'options'=>array('title'=>'Сортировать опции и значения категории'),
                                        'url'=>'Yii::app()->createUrl("/catalog/admin/category/sort/", array("id"=>$data->id))',
					'imageUrl'=>'/img/admin/small/sort.png',
                                ),
                                'catUp' => array(
                                        'label'=>'⇑',
                                        'options'=>array('title'=>'Переместить выше', 'style'=>'font-size: 14pt;'),
                                        'url'=>'Yii::app()->createUrl("/catalog/admin/category/move/", array("cid"=>$data->id, "to"=>"up"))',
                                ),
                                'catDown' => array(
                                        'label'=>'⇓',
                                        'options'=>array('title'=>'Переместить ниже', 'style'=>'font-size: 14pt;'),
                                        'url'=>'Yii::app()->createUrl("/catalog/admin/category/move/", array("cid"=>$data->id, "to"=>"down"))',
                                ),
                        ),
