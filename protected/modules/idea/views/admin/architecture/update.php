...
Yii::app()->getClientScript()->registerScript('disable_mask', "
	...
	// Засеряем и болкируем всю форму, кроме выбора типа строения и обложки
	if ($('.disabled_mask').length){
		//$('#architecture-form input, textarea, select:not(#Architecture_building_type_id)').attr('disabled', 'disabled');

		$('.disabled_mask').css({'top':'370px', 'height': formArch.height()-350});
	}


	// Сабмит формы после выбора типа строения
	$('#Architecture_building_type_id').change(function(){
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('.save_delete').click(function(){
			$.get(
				'/idea/admin/architecture/delete/id/<?php echo $model->id;?>',
				function(data){
					if (data.success) {
						document.location = '/idea/admin/architecture/list';
					}
				}, 'json'
			);
			...
			click:function(){
				...
				if ($(this).attr('id')) {
					...
					if (img_id > 0) {
						...
						$.get(
							'/idea/admin/architecture/deleteimage/id/'+img_id,
							function(response)
							{
								if (response == 'error')
		...
		$(document).ready(function(){
			$('.interior_selector').on({
				click:function(){
					...
					$.ajax({
						url	: '/idea/admin/architecture/srcImageById/id/'+imageId,
						type	: 'GET',
						async	: false,
						dataType: 'HTML',
			...
			// При загрузке страницы подгружаем фотку для выбранного пункта.
			$('.interior_selector li.current').trigger('click');
		});
		...
		$('.add_coautor_link a').click(function(){
			...
			$.ajax({
				url: "/idea/admin/architecture/addcoauthor",
		...
		$('.project_add_coautor').on({
			click:function(){
				...
				$.ajax({
					url:"/idea/admin/architecture/deletecoauthor",
		...
		$("#source-create-button").click(function(){
			$.post(
				...
				{ model_id : "<?php echo $model->id?>", model_name: 'Architecture' },
				function(response){
					if (response.success) {
...
$this->breadcrumbs=array(
	'Идеи'=>array('/idea/admin/architecture/list'),
	'Архитектура' => array('/idea/admin/architecture/list'),
	'Редактирование архитектуры'
);
?>

<h1>Редактирование архитектуры #<?php echo $model->id;?>
...
$this->widget('ext.bootstrap.widgets.BootDetailView', array(
	...
	'attributes'=>array(
		...
		array(
			'label'=>'Статус',
			'type'=>'html',
			'value'=>"<span class='label success'>".Architecture::getStatusName($model->status)."</span>",
		),
	),
));
?>
...
<div class="well" style="background-color: #F9F9F9;">
		...
		<div class="input">
			<?php $this->widget('zii.widgets.jui.CJuiAutoComplete', array(
				...
				'options'     => array(
					...
					'delay'     => 0,
					'autoFocus' => true,
					'select'    => 'js:function(event, ui) {$("#Architecture_author_id").val(ui.item.id); }',
				),
	...
	<div class="clearfix <?php if ($model->getError('image_id')) echo 'error';?>">
		...
		<div class="cover_image">
			<div>
				<?php echo Architecture::imageFormater($model->image_id);?>
			</div>
...
switch($objectTypeConst)
{
	case Architecture::BUILD_TYPE_HOUSE:
		$this->renderPartial('optionsHouse', array(
			'form' 		=> $form,
			'model' 		=> $model,
			'styles' 	=> $styles,
			'materials' 	=> $materials,
	...
	case Architecture::BUILD_TYPE_OUTBUILDING:
		$this->renderPartial('optionsOutbuilding', array(
			'model' 		=> $model,
			'form' 		=> $form,
			'materials'	=> $materials,
		));
		...
		foreach($model->getPhotoList() as $photoId) : ?>
			<div class="uploaded">
				<div class="row" style="margin-bottom: 20px;">
					<div class="span3">
						<?php echo Architecture::imageFormater($photoId);?>
						<?php echo CHtml::link('скачать оригинал', $this->createUrl('/download/productImgOriginal', array('file_id'=>$photoId))); ?>
					</div>
					<div class="span5">
						<label>Описание изображения</label>
						<textarea name="UploadedImage[desc][<?php echo $photoId;?>]"
							  class="textInput img_descript span5"
							  rows="6"
						><?php echo $imgComp->getDesc($photoId);?></textarea>
					</div>
					<div class="span5">
						<label>&nbsp;</label>
						<?php echo CHtml::link('Удалить', '#', array('class' => 'del_img btn danger', 'data-img_id' => $photoId));?>
					</div>
				</div>
			</div>
			<?php endforeach; ?>
...
<div class="well" style="background-color: #F9F9F9;">
	<?php echo CHtml::tag('h2', array('style' => 'margin-bottom: 20px;'), 'Теги', true); ?>

	<?php echo $form->textField($model, 'tag', array());?>
        <?php $this->widget('ext.EAutoSuggest.EAutoSuggest',
                array(
                        'target'=>'#Architecture_tag',
                        'url'=>'/idea/admin/architecture/acTags',
                        'options'=>array(
                                'queryParam'=>'term',
                                'minChars'=>1,
                                'keyDelay'=>250,
                                ...
                                'searchObjProps'=> "name",
                                'searchValuesProps'=> "value",
                                'selectionAdded' => 'js:function(elem, name, value){$.post("/idea/admin/architecture/createTag", {id:value, name:name, model_id: '.$model->id.'}); }',
                                'selectionRemoved'=> 'js:function(elem, value){$.post("/idea/admin/architecture/deleteTag", {id:value, model_id: '.$model->id.'}); elem.remove();}',
                        ),
                )); ?>
</div>
...
<div class="well" style="background-color: #F9F9F9;">
	<?php echo CHtml::tag('h2', array('style' => 'margin-bottom: 20px;'), 'Интерьер этого объекта', true); ?>

	<div class="row">
		<div class="span8">

			<?php // Javascript фильтр для списка интерьеров?>

			<input type="text" class="filter_interior span6" style="margin: 0 0 10px 25px;">
			<script type="text/javascript">

				var tm;
				$('.filter_interior').keyup(function(){
					clearTimeout(tm);
					var input = $(this);

					tm = setTimeout(function(){
						input.focusout().attr('disabled', 'disabled');
				...
				function filterInt(input)
				{
					...
					$('.interior_selector ul li').show();

					var expr = new RegExp('.*'+text+'.*');

					var arr = $('.interior_selector ul li');

					for (var i = 0; i < arr.length; i++)
					{
						var str = $(arr[i]).text().toLowerCase();
					...
					input.removeAttr('disabled').focusin();
				}
			</script>
...
<?php echo $form->dropDownListRow($model, 'status', Architecture::getStatusName(), array(
	'class' => 'architecture_status'
)); ?>
...
$this->widget('application.modules.idea.components.InteriorRejectedMessage.InteriorRejectedMessage', array(
	'selectorForStatus'    => '.architecture_status',
...
<div class="actions">
	<?php echo CHtml::link('Сохранить', '#', array('class'=>'btn primary large', 'id'=>'architecture-submit')); ?>
	<?php echo CHtml::link('Отмена', $this->createUrl('/'.$this->module->id.'/admin/architecture/view', array('id' => $model->id)), array('class'=>'btn default large'));?>
