...
<div class="search-form" style="display:none">
	...
	<?php echo CHtml::link('Создать ахритектуру', array('/idea/admin/architecture/create'), array('class' => 'primary btn')); ?>
	...
	$this->widget('ext.bootstrap.widgets.BootGridView', array(
		...
		'columns' => array(
			...
			array(            
                                'name'=>'name',
                                'type'=>'raw',
                                'value' => 'CHtml::link(CHtml::encode($data->name), Yii::app()->createUrl("/idea/admin/architecture/update/", array("id"=>$data->id)))',
                        ),
                        array(            
                                'name'=>'author',
                                'type'=>'raw',
			...
			array(
				'name' => 'status',
				'value' => 'Architecture::$statusNames[$data->status]',
			),
			array(
				'name'	=> 'Добавлен',
			...
			array(
				...
				'updateButtonUrl' => "Yii::app()->createUrl('/".$this->module->id."/admin/architecture/update/',	array('id' => \$data->id))",
				'deleteButtonUrl' => "Yii::app()->createUrl('/".$this->module->id."/admin/architecture/delete/',	array('id' => \$data->id))"
			),
		),
	));
	?>
...
Yii::app()->clientScript->registerScript('groupDelete', "
	...
	$('.group_delete').click(function(){
		...
		var arr = $.fn.yiiGridView.getSelection('architecture-grid');
		
		$.post('".$this->createUrl('admin/architecture/delete', array('id' => 0))."', {ids:arr}, function(){
		
			// После успешных операций, обновляем табличку.
			$.fn.yiiGridView.update('architecture-grid');
		});
