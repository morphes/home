...
<div class="shadow_block padding-18 ideas_filter">
	...
	echo CHtml::hiddenField('ideatype', Config::INTERIOR_PUBLIC);
	...
	<div class="drop_down filter_item">
		...
		if ($styles) {
			...
			{
				if ($selected['style'] == $item->id) {
					$currentStyle = $item->option_value;
					$cls = 'active';
				} else {
					$cls = '';
				}

				$styleLi .= CHtml::tag('li', array('data-value' => $item->id, 'class' => $cls), $item->option_value, true);
			}
		}
		?>
		<span class="exp_current"><?php echo $currentStyle;?><i></i></span>
		<ul class="set_input" data-callback="getCount">
			<li data-value="" <?php if ($currentStyle == '&nbsp;') echo 'class="active"';?>>&nbsp;</li>
			<?php echo $styleLi; ?>
		</ul>
		<?php echo CHtml::hiddenField('style', $selected['style'], array('id' => 'object_style'));?>
	</div>
