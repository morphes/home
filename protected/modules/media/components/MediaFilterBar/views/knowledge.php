<div class="ideas_showed">
	...
	if ($this->totalItemCount == 0) {
		...
		echo CFormatterEx::formatNumeral($this->totalItemCount, array('Показан', 'Показано', 'Показано'), true);
		echo ' ';
		echo CFormatterEx::formatNumeral($this->totalItemCount, array('материал', 'материала', 'материалов'));
	}
	?>
</div>
<form id="filter_form" class='architecture_filter' method="" action="<?php echo MediaKnowledge::getSectionLink();?>">
	<div class="shadow_block padding-18 knowledge_filter">
		...
		<div class="filter_item">
			<h3 class="block_head">Жанры</h3>
			<ul class="filter_items set_input">
				<?php
				$html = '';
				foreach (MediaKnowledge::$genreNames as $id=>$name) {
					$liOptions = array('data-id' => $id);

					if ($id == $this->filter['f_genre'])
						$liOptions['class'] = 'checked';

					$html .= CHtml::openTag('li', $liOptions);
					$html .= CHtml::tag('i', array('class' => 'genre_icon '.MediaKnowledge::$genreCssClass[$id]), '', true);
					$html .= CHtml::link($name);
					$html .= CHtml::tag('i', array('class' => 'clear_item'), '', true);
					$html .= CHtml::closeTag('li');
				}
				echo $html;
			...
			<input type="hidden" name="f_genre" value="<?php echo $this->filter['f_genre'];?>">
		</div>
		<div class="filter_item last">
			<h3 class="block_head">Кому это интересно</h3>
			<ul class="filter_items set_input">
				<?php
				$cls = ($this->filter['f_whom'] == MediaKnowledge::WHOM_USER) ? 'class="checked"' : '';
				?>
				<li data-id="<?php echo MediaKnowledge::WHOM_USER;?>" <?php echo $cls;?>><a href="#">Владельцам квартир</a><i class="clear_item"></i></li>

				<?php
				$cls = ($this->filter['f_whom'] == MediaKnowledge::WHOM_SPEC) ? 'class="checked"' : '';
				?>
				<li data-id="<?php echo MediaKnowledge::WHOM_SPEC;?>" <?php echo $cls;?>><a href="#">Специалистам</a><i class="clear_item"></i></li>
			</ul>
		...
		<input type="hidden" name="pagesize" id="elements_on_page" value="<?php echo $this->pageSize;?>">
            	...
            	<input type="hidden" name="sorttype" id="sorttype" value="<?php echo $this->sortType;?>">
