<?php $form=$this->beginWidget('ext.bootstrap.widgets.BootActiveForm',array(
	...
	<?php echo $form->textFieldRow($model,'event_id',array('class'=>'span7','maxlength'=>10)); ?>
	...
	<script>
		$(document).ready(function(){
			...
			$('#preview_image').live('change', function(){
				...
				var url = '/media/admin/mediaNew/uploadPreview/id/<?php echo $model->id;?>';
			...
			function uploadFile(file, url, callback) {
				...
				xhr.open("POST", url);
				formData.append('MediaNew[upload]', file);
				xhr.send(formData);
			}
			...
			$('.delete_preview').live('click', function(){
				var $this = $(this);
				$.post(
					'/media/admin/mediaNew/deletePreview/',
					$this.data(),
					function(response){
						if (response == 'success') {
	...
	<div class="clearfix">
		<label for="MediaNew_public_time"><?php echo $model->getAttributeLabel('content'); ?> <span class="require">*</span></label>
		<div class="input">
			<?php
				...
				'options'=>array(
					...
					'settings'	=> array(
						'addgallery' => array(
							...
							'urlGetNumber' => '/media/admin/mediaNew/getNumberOfGallery',
							// URL для сохранения описаний фоток
							'urlSaveDesc' => '/media/admin/mediaNew/saveImageDescription',
							// URL для получения всех фотографий галереи
							'urlGetImages' => '/media/admin/mediaNew/getAllGalleryImages',
							// URL загрузки изображения
							'urlUploadImage' => '/media/admin/mediaNew/uploadImageGallery/id/'.$model->id,
							// URL удаления фотографии
							'urlDeleteImage' => '/media/admin/mediaNew/deleteImageGallery',
							// URL Для получения списка всех галерей, загруженных ранее
							'urlGetListGallery' => '/media/admin/mediaNew/getListGallery',
						),
...
<div class="clearfix span9">
			...
			'options' => array(
				...
				'onRender' => 'js:function(node, nodeSpan){
					if (node.data.key=="_statusNode")
						return;
					var input=$("<input type=\'checkbox\' class=\'hide\'>").attr({"checked":node.data.select, name:"MediaNewCategory["+node.data.key+"]"});
	...
	<?php echo $form->dropDownListRow($model, 'status', MediaNew::$statusNames); ?>
	<br>
