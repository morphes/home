...
class MediaKnowledgeController extends AdminController
{
	/**
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('index','delete','uploadimage','uploadPreview', 'deletePreview',
					'UploadImageGallery', 'DeleteImageGallery', 'SaveImageDescription',
					'GetAllGalleryImages', 'GetNumberOfGallery', 'GetListGallery', 'UploadAuthorPhoto'),
				'roles'=>array(
					User::ROLE_ADMIN,
	...
	{
		$model = new MediaKnowledge;

		$model->status = MediaKnowledge::STATUS_HIDE;
	...
	public function actionUpdate($id)
	{
		...
		$sCategory = $request->getParam('MediaKnowledgeCategory', array());

		if(isset($_POST['MediaKnowledge']))
		{
			$arrayCat = array();
			...
			$model->attributes=$_POST['MediaKnowledge'];
			...
			$model->public_time = ! empty($_POST['MediaKnowledge']['public_time'])
					      ? strtotime(@$_POST['MediaKnowledge']['public_time'])
					      : time();
		...
		{
			$model->model_first = MediaNew::ARTICLE_MODEL_KNOWLEDGE;
		}

		if(empty($model->model_second))
		{
			$model->model_second = MediaNew::ARTICLE_MODEL_KNOWLEDGE;
		}

		if(empty($model->model_third))
		{
			$model->model_third = MediaNew::ARTICLE_MODEL_KNOWLEDGE;
		}
	...
	{
		$model=new MediaKnowledge('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['MediaKnowledge']))
			$model->attributes=$_GET['MediaKnowledge'];
	...
	{
		$model=MediaKnowledge::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='media-knowledge-form')
		{
	...
	public function actionUploadimage()
	{
		...
		$image->quality(85)->resize('700', '700', Image::WIDTH)->save(MediaKnowledge::UPLOAD_IMAGE_DIR . $fname);

		$url = $this->createAbsoluteUrl('/' . MediaKnowledge::UPLOAD_IMAGE_DIR . $fname);
		echo $url;
		die();
	}
	...
	{
		$model = MediaKnowledge::model()->findByPk((int)Yii::app()->getRequest()->getParam('modelId'));
	...
	public function actionGetAllGalleryImages()
	{
		...
		$knowledge = MediaKnowledge::model()->findByPk($modelId);
		if ($knowledge) {
			$models = MediaGallery::model()->findAllByAttributes(array('model' => 'MediaKnowledge', 'model_id' => $modelId, 'num' => $numGallery));
	...
	public function actionGetNumberOfGallery()
	{
		...
		$sql = "SELECT MAX(NUM) FROM media_gallery WHERE model = 'MediaKnowledge' and model_id = '".intval($modelId)."'";
	...
	public function actionGetListGallery()
	{
		...
		$knowledge = MediaKnowledge::model()->findByPk($modelId);
		if ($knowledge) {
			$sql = "SELECT image_id, num FROM media_gallery WHERE model = 'MediaKnowledge' and model_id = '{$modelId}' ORDER BY num, id";
