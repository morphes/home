...
class CustomListPager extends CBasePager
{
	...
	public function run()
	{
		...
		echo CHtml::tag('ul', $htmlOptionsMini, implode("\n", $buttons));

        if ($this->getCurrentPage() == 0) {
		    echo CHtml::tag('ul', $htmlOptionsFull, implode("\n", $this->getAllButtons()));
            ...
            $(".allpages").click(function(){
                var hidden = $(this).parents("div.pagination").find("ul:hidden");
                var current = $(this).parents("div.pagination").find("ul:not(:hidden)");
                var parent = $(this).parents("div.pagination");
	...
	protected function createPageButtons()
	{
		...
		$buttons[] = $this->createPageButton($this->prevPageLabel, (($page = $currentPage - 1) < 0)
			? 0
			: $page, self::CSS_PREVIOUS_PAGE, $currentPage <= 0, false, true);
		...
		$buttons[] = $this->createPageButton($this->nextPageLabel, (($page = $currentPage + 1) >= $maxPage)
			? $maxPage
			: $page, self::CSS_NEXT_PAGE, $currentPage >= $maxPage, false, true);
	 ...
	 */
	protected function createPageButton($label, $page, $class, $hidden, $selected, $noindex = false)
	{
		if ($selected) {
			return '<li>' . CHtml::tag('strong', array(), $label) . '</li>';
		} else {
			...
			if ($noindex === true) {
				$res = '<li><noindex>' . CHtml::link($label, $url, $options) . '</noindex></li>';
			} else {
				$res = '<li>' . CHtml::link($label, $url, $options) . '</li>';
			}
			return $res;
		}
	}
