...
class Catalog2Rules extends ParseUrlAbstract
{
    ...
    {
        Yii::import('application.modules.catalog2.models.Catalog2ActiveRecord');
        parent::__construct($manager, $request, $pathInfo, $rawPathInfo, $pathArray);
    }
	...
	public function getRouteOneWord()
	{
		if ($this->cityId === null) {
			...
			return 'catalog2/category/list/id/0';
		}
		...
		if (!empty($cityName) && !$this->isAjax) {
			...
			$this->request->redirect('/products/' . $cityName . $tail);
		} else {
			return 'catalog2/category/list/id/0';
		}
	}
	...
	public function getRouteTwoWord()
	{
		...
		if ($this->matches[2] == 'stores') {
			...
			if (!empty($cityName) && !$this->isAjax) {
				...
				$this->request->redirect('/products/stores/' . $cityName . $tail);
			}
		}
		...
		if ($categoryId) {
			...
			if ($this->cityId === null) {
				...
				return 'catalog2/category/list/id/' . $categoryId;
			}
			...
			if (!empty($cityName) && !$this->isAjax) {
				...
				if (!empty($_REQUEST))
					$tail = '?' . http_build_query($_REQUEST);
				$this->request->redirect('/products/' . $cityName . '/' . $this->matches[2] . $tail);
			} else {
				return 'catalog2/category/list/id/' . $categoryId;
			}
		}
		...
		if ($this->matches[2] == 'stores') {
			...
			if (isset($_REQUEST['map']) && $_REQUEST['map'] == 'show'){
				return 'catalog2/store/listmap';
			} else {
				return 'catalog2/store/list';
			}
	...
	public function getRouteThreeWord()
	{
		...
		if($this->matches[2] == 'vendor' && isset($this->matches[3]) && preg_match("/^\d+$/", $this->matches[3]) == 1) {
			...
			if(count($this->matches) == 4)
				return 'catalog2/vendor/index/id/' . $this->matches[3];
		...
		if ($categoryId && preg_match('/^[\w0-9-]+$/', $this->matches[3])) {
			...
			Yii::import('application.modules.catalog2.models.Vendor');
			...
			if ($vendor) {
				return 'catalog2/category/list/id/'. $categoryId . '/vendors[]/' . $vendor->id;
			}
		}
		...
		if (preg_match("/^\d+$/", $this->matches[3]) == 1) {
			 ...
			 */
			return 'catalog2/product/index/id/' . $this->matches[3];
	...
	public function getRouteFourWord()
	{
		...
		if ($categoryId = $this->getCategoryId($this->matches[2])) {
			...
			return 'catalog2/product/' . $this->matches[4] . '/id/' . $this->matches[3];
		}

		if ($this->matches[2] == 'vendor') {
			// внутренние страницы карточки производителя
			return 'catalog2/vendor/' . $this->matches[4] . '/id/' . $this->matches[3];
		}
	...
	static public function createUrl($manager, $route, $params, $ampersand)
	{
		...
		if ($route === 'catalog2/category/list') {
			...
			if (isset($params['eng_name'])) { // существует название категории
				...
				if ($params['id'] == 0) {
					...
					unset($params['id']);
					return 'products?' . Yii::app()->getUrlManager()->createPathInfo($params, '=', $ampersand);
				}
			...
			if (isset($cityName)) {
				...
				if ($query)
					return 'products/' . urlencode($cityName) . '/' . urlencode($engName) . '?' . $query;
				else
					return 'products/' . urlencode($cityName) . '/' . urlencode($engName);
				...
				if ($vendorCountry) {
					...
					if ($query) {
						return 'products/' . urlencode($engName) . '/' . urlencode($vendorCountryName) . '?' . $query;
					} else {
						return 'products/' . urlencode($engName) . '/' . urlencode($vendorCountryName);
					}
				}

			} else {
				if ($query)
					return 'products/' . urlencode($engName) . '?' . $query;
				else
					return 'products/' . urlencode($engName);
			}
		}
		// страница товара
		if ($route === 'product2' && isset($params['id'])) {
			...
			if ( isset($params['category_id'])) {
				...
				$category = Yii::app()->dbcatalog2->createCommand()->select('c.eng_name')->from('cat_product p')
			...
			if (isset($params['action'])) {
				...
				if ($params['action'] === 'index' && isset($params['cid']))
					return 'products/' . $category . '/' . $params['id'] . '/' . $params['action'] . '?cid=' . $params['cid'] . '&' . $postfix;
				elseif ($params['action'] === 'index')
					return 'products/' . $category . '/' . $params['id'] . $delimiter . $postfix;
				...
				elseif ($params['action'] === 'storesInCity' && isset($params['cid']))
					return 'products/' . $category . '/' . $params['id'] . '/' . $params['action'] . '?cid=' . $params['cid'] . '&' . $postfix; // любая другая страница карточки товара
				else
					return 'products/' . $category . '/' . $params['id'] . '/' . $params['action'] . $delimiter . $postfix;
			}
		}
		if ($route === 'catalog2/vendor' && isset($params['id'])) {
			...
			if (isset($params['action'])) {
				...
				// главная страница производителя
				if ($action === 'index') {
					return 'products/vendor/' . $id;
				} elseif ($query) {

					// внутренние страницы производителя со строкой get параметров (если есть)
					return 'products/vendor/' . $id . '/' . $action . '?' . $query;
				} else {

					return 'products/vendor/' . $id . '/' . $action;
				}
			}
		}

		// Генерация ссылок на страницы списка магазинов
		if ($route === 'catalog2/stores') {
			...
			if (isset($params['map']) && $params['map'] == 'show') {
				return 'products/stores' . $cityName . $query;
			} else {
				return 'products/stores' . $cityName . $query;
			}
		}
	...
	{
		// поиск запрошенной категории в базе
		return Yii::app()->dbcatalog2->createCommand()->select('id')->from('cat_category')
	...
	{
		// поиск запрошенной категории в базе
		return Yii::app()->dbcatalog2->createCommand()->select('eng_name')->from('city')
	...
	private function _categoryCountry()
	{
		...
		Yii::import('application.modules.catalog2.models.Vendor');
		Yii::import('application.modules.catalog2.models.Product');
		...
		if ($foundCountryId == 0) {
			...
			return 'catalog2/category/list/id/' . $categoryId . '/vendor_country/' . $foundCountryId;
		}
	}
