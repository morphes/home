...
class XmlParserFurnish extends XmlParserAbstract implements  XmlParserInterface
{
    const PARSER_ID = 1;
    ...
    public function __construct($file)
    {
        ...
        $this->initCategoryMapper('furnish_category_mapping.json');
        $this->setFile($file);
        $this->load();
        $this->validate();
    }
    ...
    public function parse($storeId)
    {
        ...
        foreach($this->_object->shop->offers->offer as $offer) {
            ...
            $storeInnerId = (int) (isset($offer->attributes()['id']) ? $offer->attributes()['id'] : null);
            if (!$storeInnerId) {
                continue;
            }
            ...
            $storePrice->price = (float) $offer->retailprice;
            $storePrice->save();
    ...
    {
        if (!isset($this->_object->shop->name) || $this->_object->shop->name != 'The Furnish') {
            throw new CException('Invalid store name in xml file');
        }
        if (!isset($this->_object->shop->url) || $this->_object->shop->url != 'http://www.thefurnish.ru') {
