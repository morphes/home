...
class XmlParserHomeme extends XmlParserAbstract implements  XmlParserInterface
{
    const PARSER_ID = 4;
    ...
    public function __construct($file)
    {
        ...
        $this->initCategoryMapper('homeme_category_mapping.json');
        $this->setFile($file);
        $this->load();
        $this->validate();
    }
    ...
    public function parse($storeId)
    {
		...
		echo "Missing storeInnerId";
            ...
            if (!$category) {
		echo "Missing mapping for category" . $offer->categoryId;
                continue;
            }
    ...
    {
        if (!isset($this->_object->shop->name) || $this->_object->shop->name != 'HomeMe.ru') {
            throw new CException('Invalid store name in xml file');
        }
        if (!isset($this->_object->shop->url) || $this->_object->shop->url != 'http://www.homeme.ru') {
