...
class XmlParserPostelDeluxe extends XmlParserAbstract implements  XmlParserInterface
{
    const PARSER_ID = 7;
    ...
    public function __construct($file)
    {
        ...
        $this->initCategoryMapper('posteldeluxe_category_mapping.json');
        $this->setFile($file);
        $this->load();
        $this->validate();
    }
    ...
    {
        if (!isset($this->_object->shop->name) || $this->_object->shop->name != 'Постель Делюкс') {
            throw new CException('Invalid store name in xml file');
        }
        if (!isset($this->_object->shop->url) || $this->_object->shop->url != 'http://postel-deluxe.ru') {
