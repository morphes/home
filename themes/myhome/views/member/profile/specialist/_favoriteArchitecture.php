...
<?php if ($countModels > 0): ?>
	<div class="favorite_item">
		<h3>Архитектура<span class="items_count quant_f"><?php echo $countModels;?></span></h3>
			...
			<div class="gallery-210">
				...
				<div class="item">
					...
					<div class="item_photo">
						<?php
							...
							if (isset(Config::$rolesAdmin[$proj->author->role]))
								$projectUrl = Yii::app()->createUrl("/idea/", array('architecture' => $proj->id));
							else
								$projectUrl = Yii::app()->createUrl("/users/{$proj->author->login}/project/{$proj->service_id}/{$proj->id}");
						?>
						<a href="<?php echo $projectUrl; ?>">
							<?php
							if ($proj instanceof Architecture) {
								$src = $proj->getPreview('crop_210');
							} else {
								$src = '/'.$proj->getPreview(Config::$preview['crop_210']);
							}
							?>
							<img align="left" width="210" height="210" alt="<?php $proj->name; ?>" src="<?php echo $src; ?>">
						</a>
					</div>
					<div class="item_descript">
						...
						<a class="item_name" href="<?php echo Yii::app()->createUrl("/users/{$proj->author->login}/project/{$proj->service_id}/{$proj->id}"); ?>">
							<?php echo $proj->name; ?>
						</a>
					</div>
