		...
		<div class="-col-8">
			...
			<h1 class="-gutter-bottom-hf"><?php echo $store->name;?>
				...
				if ($isOwner && !empty($store->about)) { ?>
					<a href="<?php echo $this->createUrl('/catalog/profile/storeUpdate', array('id' => $store->id));?>" class="-push-right -small -normal -gray -icon-pencil-xs">Редактировать описание</a>
				<?php } ?>
			...
			<h2 class="-gutter-bottom-hf">Мы предлагаем
				...
				if ($isOwner && !$store->checkEmptyShowcase()) { ?>
					<a href="<?php echo $this->createUrl('/catalog/profile/storeShowcase', array('id' => $store->id)); ?>" class="-block -gutter-top -push-right -small -normal -gray -icon-pencil-xs">Обновить товары</a>
				<?php } ?>
			</h2>
					...
					<div class="-grid -inset-top-hf">
						<div class="-col-8 -tinygray-box -inset-all block-placeholder">
							<p class="-text-align-center -inset-all -gutter-top-hf -large">
								...
								<br>
								<span class="-block -gutter-top-hf"><a href="<?php echo $this->createUrl('/catalog/profile/storeShowcase/', array('id' => $store->id, 'from_card' => 1)); ?>" class="-red">Редактировать витрину товаров</a></span>
							</p>
						</div>
					</div>
				<?php } ?>
