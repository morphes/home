		...
		<div class="-col-4 -skip-4 -pass-3 -inset-bottom -text-align-center">
			<form id="authForm" method="post" action="/site/login" name="">
                <?php if ($user->loginErrorCode() === UserIdentity::NOTICE_TMPPASS_REQUIRED || $user->loginErrorCode() === UserIdentity::ERROR_TMPPASS) : ?>
                    ...
                    <p>
                        На ваш email, указанный при регистрации, отправлено письмо с разовым паролем для входа. Введите отправленный вам разовый пароль.
                        Данная мера необходима только при попытке входа в административный аккаунт с неизвестного IP адреса.
                    </p>

                    <p class="p-login-name">
                        <label for="tmpPass">Разовый пароль</label><br>
                        <input type="text" name="User[tmpPass]" class="textInput" tabindex="1" value="<?php echo $user->tmpPass; ?>" />
                    </p>
