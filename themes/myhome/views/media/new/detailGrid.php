	...
	<div class="-grid">
		<div class="-col-12">
			<ul class="-menu-inline -breadcrumbs">
				...
				<li><a href="<?php echo MediaNew::getSectionLink(); ?>">Новости</a></li>
			</ul>
		</div>
...
<div class="-grid-wrapper page-content">
	<div class="-grid">
		<div class="-col-9 -inset-right">
				...
				<p class="-small">
					...
					if ($themes) {
						...
						foreach ($themes as $key => $theme) {
							$html .= CHtml::link($theme->name, $this->createUrl(MediaNew::getSectionLink()));
			...
			if ($this->beginCache(
				...
				array(
					'dependency' => array(
						...
						'sql'   => 'SELECT update_time FROM ' . MediaNew::model()->tableName(). ' WHERE id = ' . $model->id
					)
				)
			)
			) {
			...
			<?php if ($this->beginCache('JOURNAL:RELATED:CATEGORIES:NEWS:' . $model->id, array('duration' => 7200))) { ?>
...
<!--				--><?php //if ($this->beginCache('JOURNAL:RELATED:PRODUCTS:NEWS:' . $model->id, array('duration' => 7200))) { ?>
