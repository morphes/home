...
<?php Yii::app()->clientScript->registerScriptFile('/js-new/CCatalog2.js'); ?>
<?php Yii::app()->clientScript->registerScriptFile('/js-new/catalog2.js'); ?>
	...
	<div class="-col-5 summary">
		...
		<span class="-block">Магазин
         <noindex>
				<a class="-skyblue"
                   rel="nofollow"
				   href="<?php echo Amputate::absoluteUrl($store->site); ?>"><?php echo $store->name ?></a></span>
        </noindex>
	</div>
	<noindex>
 ...
 * -----------------------------------------------------------------------------
 */
$this->renderPartial('//catalog2/product/_description',
	array('model' => $model)
); ?>
...
<?php if ($similars) {
	$this->renderPartial('//catalog2/product/_similarProduct',
...
if ($store->type != Store::TYPE_ONLINE) {
	$this->renderPartial('//catalog2/product/_cityStoriesPopap',
		array(
			'stores'      => $stores,
			'store'       => $store,
			'model'       => $model,
...
if (count($sorted) > 1) {
	$this->renderPartial('//catalog2/product/_citiesWithProduct', array('sorted' => $sorted, 'model' => $model));
}
?>
</div>
