...
<div class="pathBar">
	<?php $this->widget('application.components.widgets.EBreadcrumbs', array(
		'encodeLabel' => false,
		'links'       => array(
			'Магазины' => $this->createUrl('/catalog2/stores')
		),
	));?>
	...
	<?php if ($isOwner) : ?>
		<a class="edit"
		   href="<?php echo $this->createUrl('/catalog2/profile/storeUpdate/', array('id' => $model->id)); ?>"><i></i>Редактировать</a>
	<?php endif; ?>
...
<div class="shop_info">
	...
	<div class="shop_contacts">
		...
		<ul class="timetable">
			...
			if ($model->site && $model->tariff_id != Store::TARIF_FREE && $model->tariff_id != null) {
				...
				$links[] = CHtml::link(
					Amputate::absoluteUrl(htmlentities($model->site, null, 'UTF-8')),
					$this->createUrl(
						'viewSite',
						array(
							'store_id' => $model->id
			...
			<?php
			echo Chtml::openTag('noindex');
				echo !empty($links) ? implode('<br> ', $links)
				: '';
			echo Chtml::closeTag('noindex');
			?>
                </span>
	</div>
	<div class="shop_rubrics">
		...
		<?php if ($chain) : ?>
			<div class="chain_stores">
				<span>Входит в сеть:</span> <?php echo CHtml::link($chain->name, $this->createUrl('/catalog2/chain/index', array('id' => $chain->id))); ?>
			</div>
		<?php endif; ?>
		...
		<?php if ($model->tariff_id != Store::TARIF_FREE) : ?>
			<div class="catalog_sections">
				...
				foreach ($model->getUsedCategory() as $cat_id) {
					...
					$links[] = CHtml::link($catName, $this->createUrl('/catalog2/store/products/', array('id' => $model->id, 'category_id' => $cat_id)));
				}
				echo implode(', ', $links);
				?>
			</div>
	...
	<div class="item_rating">
		...
		<?php echo CHtml::link(($feedback_qt == 0) ? 'Нет отзывов'
			: CFormatterEx::formatNumeral($feedback_qt, array('отзыв', 'отзыва', 'отзывов')), $this->createUrl('/catalog2/store/feedback', array('id' => $model->id))); ?>
		<?php echo CHtml::link('Оставить отзыв', $this->createUrl('/catalog2/store/feedback', array('id' => $model->id)) . '#product_comment_form'); ?>
	</div>
	<div class="clear"></div>
</div>
<div class="shop_left">
	...
	<?php if ($model->tariff_id != Store::TARIF_FREE): ?>
		<div class="other_models">
			...
			<span class="comments_quant"><?php echo $model->productQt; ?></span>
			      ...
			      style="display: inline;">
                                <a href="<?php echo $this->createUrl('/catalog2/store/products/', array('id' => $model->id)); ?>">Все</a>
                                <span>&rarr;</span>
                        </span>
			<?php if ($model->checkEmptyShowcase() && $isOwner) : ?>
				<div class="catalog_items_list_small store empty">
					...
					витрину<br>
					<a class="edit hover"
					   href="<?php echo $this->createUrl('/catalog2/profile/storeShowcase/', array('id' => $model->id, 'from_card' => 1)); ?>"><i></i>Редактировать
																					 витрину
																					 товаров</a>
				</div>
			<?php else : ?>
				...
				<?php if ($isOwner) : ?>
					<a class="edit"
					   href="<?php echo $this->createUrl('/catalog2/profile/storeShowcase/', array('id' => $model->id)); ?>"><i></i>Редактировать
																		       витрину
																		       товаров</a>
				<?php endif; ?>
		...
		<?php if (!empty($model->about) || $isOwner) : ?>
			<div class="shop_description">
				...
				<?php if ($isOwner) : ?>
					<a class="edit"
					   href="<?php echo $this->createUrl('/catalog2/profile/storeUpdate/', array('id' => $model->id)); ?>"><i></i>Редактировать
																		     описание</a>
				<?php endif; ?>
				...
				<?php if (empty($model->about) && $isOwner) : ?>
					<div class="empty">
						<a class="edit hover"
						   href="<?php echo $this->createUrl('/catalog2/profile/storeUpdate/', array('id' => $model->id)); ?>"><i></i>Добавить
																			     описание
																			     магазина</a>
					</div>
				<?php endif; ?>
	...
	<div class="buyers_opinion">
		...
		<?php if (!$feedbacks->getTotalItemCount()) : ?>
			...
			<p>Пока нет ни одного отзыва. Вы можете стать первым!
				...
				<?php echo CHtml::link('Оставьте свой отзыв', $this->createUrl('/catalog2/store/feedback', array('id' => $model->id)) . '#product_comment_form'); ?>
                        </span>
			</p>
		<?php else : ?>
			<h3 class="headline">Отзывы о магазине</h3>
			...
			<span class="all_elements_link"
			      style="display: inline;">
                        <?php echo CHtml::link('Все', $this->createUrl('/catalog2/store/feedback', array('id' => $model->id))); ?>
				<span>&rarr;</span>
                    </span>
				...
				<?php foreach ($model->getVendors() as $vendor) : ?>
					...
					<li>
						<a title="<?php echo $vendor->name; ?>"
						   href="<?php echo $this->createUrl('/catalog2/store/products/', array('id' => $model->id, 'vendor_id' => $vendor->id)); ?>">
							<?php
