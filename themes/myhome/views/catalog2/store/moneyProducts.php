		...
		<div class="-col-9">
			...
			<div class="-grid page-controls-top">
				...
				<div class="-col-wrap -small sorting">
					...
					<a href="<?php echo Yii::app()->createUrl('catalog2/store/moneyProducts', array('show' => 'all') + $_GET);?>" class="-gutter-left-qr -acronym <?php if ($showType != 'discount') echo 'current';?>">все</a>
					<a href="<?php echo Yii::app()->createUrl('catalog2/store/moneyProducts', array('show' => 'discount') + $_GET);?>" class="-gutter-left-qr -acronym <?php if ($showType == 'discount') echo 'current';?>">со скидкой</a>
				</div>
			...
			if (!empty($data)) {
				if ($viewType == 2) {
					...
					foreach ($data as $index => $item) {
						$this->renderPartial('//catalog2/category/_product3colGrid', array(
							'data'     => $item,
							'index'    => $index,
							'qt'       => $qt,
							'store_id' => $store->id
					...
					foreach ($data as $item) {
						$this->renderPartial(
							'//catalog2/category/_product2colGrid',
							array(
								'data'     => $item,
								'store_id' => $store->id,
								'class'    => '-col-wrap'
