	...
	this.initShopList = function(){
		...
		function _storeAddress(){
				...
				if(!block.hasClass('animated')){
					if(block.hasClass('showed')) {
						...
						if(block.hasClass('loaded')){
							...
							$.ajax({
								url: '/catalog2/store/ajaxGetBySymbol/id/' + block.attr('id') + '/lang/' + block.attr('data-lang') + '/cityId/' + cityId,
								method: 'GET',
								dataType: 'JSON',
								success: function (data) {
									block
			...
			list.on('click', '.address-list',function(){
				...
				$.ajax({
					url: '/catalog2/store/ajaxGetChain/id/' + id,
					method: 'GET',
					dataType: 'JSON',
					success: function(data){
						$('#popup-stores-list')
		...
		function _storesSearch(){
			list.find('input').keyup(function(e){
				...
				function _loadSearchResults(){
					...
					$.ajax({
						url: '/catalog2/store/ajaxSearchForMap',
						method: 'POST',
						data: {
							text: val,
							cityId: cityId,
